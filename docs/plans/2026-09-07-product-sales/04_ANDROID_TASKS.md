# 단일 에이전트 작업지시서 — Android 판매관리

작성일: 2026년 9월 7일 · 단일 에이전트 실행안 개정 2026년 9월 8일 · 대상: `android/voicecapSMS`
...
이 단계의 수정 범위는 `android/voicecapSMS/**`이며 결과는 Android UI, 카메라, API 연결, 기기 테스트와 Android 완료 보고다. 공통 계약, `supabase/**`, `src/**`, `server/**`, `desktop/**` 변경이 필요하면 Android 코드 안에서 임시 우회하지 않는다. 현재 Android 작업을 멈추고 [공통 계약](02_SHARED_CONTRACT.md), fixture, [공통 작업서](03_WEB_BACKEND_PC_TASKS.md)와 관련 테스트를 먼저 갱신한 뒤 이 단계로 돌아온다.

서버 응답을 Android에 맞춰 임의로 해석하거나 PC 도우미에 직접 연결하지 않는다. Android의 `127.0.0.1`은 휴대폰 자신이므로 PC 댓글 도우미 주소로 사용할 수 없다.
...
app/src/main/java/com/voicecap/sms/
  ...
  sales/
    ...
    FakeSalesRepository.java         CORE-01에서 확정한 fixture로 개발
...
### ANDROID-01. 개발 환경과 SMS 기준 상태 확보
...
2. Gradle/JDK/SDK 조합을 확인하고 기존 debug 빌드·lint 결과를 남긴다. compileSdk와 targetSdk 불일치가 새 라이브러리를 막으면 현재 저장소 기준과 호환성을 확인하고 Android 범위 안에서 조정한다.
...
### ANDROID-02. 탭과 생명주기 기반 화면 뼈대

**선행:** ANDROID-01 완료. **작업:** AndroidX Fragment/Activity와 RecyclerView 의존성을 실제 Gradle 조합에서 검증하고 버전을 고정한다.
...
### ANDROID-03. 공통 모델·모의 API·요청 상태 보존

**선행:** CORE-01에서 `contracts/product-sales/v1/` fixture와 JSON Schema 검증을 완료했다. 미완료이면 임의 응답을 확정하지 말고 ANDROID-02까지만 진행한다.

1. `SalesRepository` 아래 모의/실제 구현을 나눈다. fixture는 공통 원본을 읽거나 빌드에서 복사하며 Android 전용으로 다른 계약을 만들지 않는다.
...
### ANDROID-04. 판매관리 기본 화면과 설정

**선행:** ANDROID-02, ANDROID-03 완료. **사용 action:** `get-bootstrap`, `get-sales-feed`, `update-settings`.
...
7. 판매 권한이 없으면 웹 기기관리에서 관리자가 권한을 켜야 함을 안내한다. `list-devices`, `update-device-capabilities`, `set-output-device`는 CORE 단계의 웹 기기관리 흐름이며 Android가 자신의 권한을 임의로 올리거나 출력 대상을 바꾸지 않는다.

**산출물:** 판매 탭·공통 설정 화면. **완료:** 설정 조합과 빈 상태에서 허용되지 않은 등록을 실행하지 않고 기존 저장 상품이 사라지지 않는다.

### ANDROID-05. 최신 댓글과 구매자별 선택·수량

**선행:** ANDROID-04 완료. **사용 action:** `get-sales-feed`.
...
8. 새 댓글이 0건이어도 응답의 summary, buyerStats, activeProduct, revision을 반영한다. 댓글 cursor가 그대로라는 이유로 웹에서 바뀐 가격·구매자·합계를 무시하지 않는다. 100명을 넘는 조회는 공통 Schema와 페이지 규칙으로 나누며 UI의 확정 통계를 단말에서 추정하지 않는다.

**산출물:** 댓글 목록·다중 선택·수량 조절·예상 통계. **완료:** 같은 구매자 댓글 3개 선택은 기본 1개이고 수량을 2로 고치면 예상 수량만 2개 증가한다.

### ANDROID-06. 상품등록과 전면 카메라 2초 촬영

**선행:** ANDROID-03, ANDROID-04 완료, CORE-04의 이미지 업로드 계약과 fixture 준비. **사용 action:** `prepare-product`, `update-product-draft`, `commit-product`.
...
### ANDROID-07. 판매등록 완료와 출력 상태

**선행:** ANDROID-05, ANDROID-06와 CORE-07 완료, 실제 판매 API 준비. **사용 action:** `search-buyers`, `confirm-buyer`, `commit-sales`, `get-operation`, `get-print-status`.
...
### ANDROID-08. 상품별 판매내역과 구매자 수정

**선행:** ANDROID-07과 CORE-09 완료, 수정 preview fixture/API 준비. **사용 action:** `list-session-products`, `get-product-sales`, `search-buyers`, `confirm-buyer`, `prepare-product-image`, `preview-product-change`, `commit-product-change`.
...
3. 사진 변경은 prepare-product-image로 현재 productId의 수정용 imageUpload와 imageId를 받는다. ANDROID-06의 전면 카메라 제어와 업로드 처리를 재사용하고 업로드한 imageId를 proposedProduct.imageId에 넣는다. 사진 업로드만으로 상품을 수정하지 않으며 기존 상품 등록용 prepare/commit으로 새 상품을 만들지 않는다.
4. `preview-product-change`에 proposedProduct, proposedSales, expectedProductRevision, expectedSalesRevision을 보낸다. proposedSales 행의 saleId/expectedRevision과 추가·취소 구조는 공통 JSON Schema를 그대로 따른다.
...
### ANDROID-09. 실제 API 연결과 오류 처리

**선행:** ANDROID-03~ANDROID-08 모의 화면과 CORE-03~CORE-10 완료, 테스트 환경과 판매 권한 준비. **작업:** Repository만 실제 구현으로 전환해 같은 화면을 검증한다.
...
### ANDROID-10. 회귀 검증·배포용 빌드·완료 보고

**선행:** 모든 ANDROID 티켓과 CORE-01~CORE-10 완료, PC 도우미를 포함한 테스트 환경 준비. [공통 인수시험](05_ACCEPTANCE_TESTS.md)의 Android 관련 항목을 실행한다.
...
4. ANDROID-01의 SMS 기준 항목을 다시 실행한다. 역할 요청, 권한 거절, 업무 문자 필터링, 수신·발신·15분 작업을 포함한다.
...
아래 명령은 현재 기능 브랜치의 `android/voicecapSMS`에서 실행한다. 테스트 경로는 에이전트가 작성한 테스트를 검증하며 설치·운영 배포를 자동 승인하는 명령이 아니다.
...
**완료 보고:** 변경 파일/커밋, 사용 API 버전, fixture 버전, Gradle·SDK·기기 정보, 빌드 결과, 인수시험 증거, 미해결 문제와 재현 절차, 토큰 없는 진단 로그, 테스트 APK/AAB 위치를 기록한다. 최종 완료 조건은 서버·웹과 같은 데이터·동일 계산·동일 수정 결과·중복 없는 판매를 확인하고 기존 SMS 기능이 유지되는 것이다.
