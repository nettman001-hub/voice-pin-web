...
public class SmsBridgeView extends ScrollView {
    public static final int TONE_INFO = 1;
    public static final int TONE_SUCCESS = 2;
    public static final int TONE_WARNING = 3;
    ...
    public SmsBridgeView(MainActivity activity) {
        super(activity);
        this.activity = activity;
        setFillViewport(true);
        setClipToPadding(false);
        setBackgroundColor(SLATE_50);
        addView(buildContent(), new LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT));
    }
    ...
    private View buildContent() {
        ...
        LinearLayout brandCopy = new LinearLayout(getContext());
        ...
        LinearLayout guide = new LinearLayout(getContext());
        ...
        openWeb.setOnClickListener(v -> {
            try {
                activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("https://www.voicecap.shop/my")));
        ...
        setup.setOnClickListener(v -> activity.beginProtectedSetup());
        ...
        privacy.setOnClickListener(v -> activity.showDataUseNotice(false));
        ...
        settings.setOnClickListener(v -> activity.startActivity(new Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS, Uri.parse("package:" + activity.getPackageName()))));
        managementCard.addView(settings);

        Button revoke = button("이 휴대폰 연결 해제", BUTTON_DANGER);
        revoke.setOnClickListener(v -> {
            BridgeClient.revokeDevice(activity);
    ...
    public void updateSetupStatus() {
        if (!BridgePreferences.configured(activity)) setStatus("웹앱에서 기기 연결 코드를 만든 뒤 이 휴대폰을 연결해 주세요.", TONE_WARNING);
        else if (!BridgePreferences.hasDataTransferConsent(activity)) setStatus("문자 정보 전송 동의가 필요합니다.", TONE_WARNING);
        else if (!SmsRoleUtils.isDefaultSmsApp(activity)) setStatus(R.string.status_role_required, TONE_WARNING);
        else if (!SmsRoleUtils.hasSmsPermissions(activity)) setStatus(R.string.status_permission_required, TONE_WARNING);
        else setStatus(R.string.status_bridge_ready, TONE_SUCCESS);
    }

    public void setStatus(String value, int tone) {
        if (status == null) return;
        status.setText(value);
        applyStatusTone(tone);
    }

    public void setStatus(int stringResource, int tone) {
        setStatus(activity.getString(stringResource), tone);
    }
    ...
    private void pairDevice() {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            boolean paired = BridgeClient.claimDevice(activity.getApplicationContext(), code, deviceName);
    ...
    private void syncInBackground() {
        if (!BridgePreferences.configured(activity)) {
            setStatus("먼저 이 휴대폰을 판매자 계정에 연결해 주세요.", TONE_WARNING);
            return;
        }
        if (!BridgePreferences.hasDataTransferConsent(activity) || !SmsRoleUtils.isDefaultSmsApp(activity) || !SmsRoleUtils.hasSmsPermissions(activity)) {
            activity.beginProtectedSetup();
            return;
        }
        setStatus("구매정보 문자와 발송 대기 문자를 처리하는 중...", TONE_INFO);
        Executors.newSingleThreadExecutor().execute(() -> {
            String result = SmsSyncManager.syncNow(activity.getApplicationContext());
            activity.runOnUiThread(() -> setStatus(result, TONE_INFO));
        });
    }
