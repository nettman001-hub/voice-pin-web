  ...
  return (
    <div className="p-3.5 sm:p-6 max-w-6xl mx-auto space-y-4 sm:space-y-6">
      <div className="text-center space-y-1.5 sm:space-y-2">
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-slate-900">맞춤형 플랜을 선택하세요</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 pt-2 sm:pt-4">
        {SUBSCRIPTION_PLANS.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`relative rounded-3xl p-5 sm:p-6 bg-white border flex flex-col justify-between transition transform hover:-translate-y-1 ${
                plan.isPopular
            ...
            >
              ...
              <div>
                ...
                <div className="mt-3 sm:mt-4 flex items-baseline">
                  <span className="text-2xl sm:text-3xl font-black text-slate-900">{plan.priceMonth.toLocaleString()}</span>
                  <span className="text-xs text-slate-500 ml-1">원 / 월</span>
                </div>

                <div className="mt-4 sm:mt-6 space-y-1.5 sm:space-y-2 text-xs text-slate-700 border-t border-slate-100 pt-3 sm:pt-4">
              ...
              <div className="mt-5 sm:mt-6 pt-4 border-t border-slate-100">
                <button
                  ...
                  disabled={isCurrent}
                  className={`w-full py-3 sm:py-2.5 rounded-xl font-bold text-xs flex items-center justify-center space-x-1.5 transition active:scale-95 ${
                    isCurrent
