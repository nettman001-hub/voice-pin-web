    ...
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 p-6 rounded-3xl shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">판매자 음성 학습 & 훈련</h1>
        ...
        {/* 훈련 완료 게이지 */}
        <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center space-x-4 min-w-[220px]">
          <div className="w-12 h-12 rounded-xl bg-brand-100 text-brand-700 flex items-center justify-center font-black text-base">
            {progressPercent}%
          </div>
          <div>
      ...
      {countdown !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-md p-4">
          <div className="bg-white border border-slate-200 p-8 rounded-3xl max-w-md w-full text-center space-y-6 shadow-2xl animate-in zoom-in-95">
            <span className="text-xs font-bold text-brand-600 uppercase tracking-widest">
              녹음 준비 카운트다운
            </span>
            <div className="text-7xl font-black text-slate-900 font-mono animate-bounce">
              {countdown}
            </div>
            <p className="text-sm font-bold text-slate-800">
              "{activeSentence?.sentence}"
            </p>
            <p className="text-xs text-slate-500">카운트다운 후 문장을 또박또박 읽어주세요.</p>
          </div>
        </div>
      )}
      ...
      {isRecording && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-md p-4">
          <div className="bg-white border border-rose-200 p-8 rounded-3xl max-w-md w-full text-center space-y-6 shadow-2xl">
            <div className="w-20 h-20 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center mx-auto animate-pulse">
              <Mic className="w-10 h-10" />
            </div>
            <h3 className="text-xl font-black text-slate-900">🎙️ 음성을 녹음하고 있습니다...</h3>
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
              <p className="text-base font-bold text-brand-700">
                "{activeSentence?.sentence}"
              </p>
            </div>
            <p className="text-xs text-slate-500">자연스러운 목소리로 위 문장을 읽어주세요.</p>
          </div>
        </div>
      )}
          ...
          {trainingSentences.map((item, idx) => (
            ...
            >
              <div className="flex items-start space-x-3.5">
                ...
                <div>
                  <div className="flex items-center space-x-2">
                    <span className="text-sm font-bold text-slate-900">"{item.sentence}"</span>
                    {item.isCompleted ? (
                      <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold border border-emerald-300 flex items-center space-x-1">
