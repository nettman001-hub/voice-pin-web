...
> **상세 종합 문서**: [PROJECT_HANDOVER.md](file:///c:/dev/voicecap-web/PROJECT_HANDOVER.md)  
...
| **데스크톱 도우미** | `v1.3.4` (Windows x64 Electron 44, Xprinter 감열식 출력, 하이브리드 STT) |
| **안드로이드 앱** | `v1.3.1` (Java 17, `shop.voicecap.smsbridge`, compileSdk 35 / targetSdk 35) |
...
2. **Supabase DDL 및 Edge Functions (`supabase/`)**:
   - `supabase/migrations/20260907_product_sales.sql` (세션, 상품, 판매, 구매자, 출력 큐, 기기 권한).
...
4. **데스크톱 댓글 도우미 & 영수증 프린터 (`desktop/comment-helper/` & `server/`)**:
   - `salesPrintWorker.cjs` / `salesPrintBridge.js`: 클라우드 인쇄 큐 폴링 및 Xprinter 영수증 자동 인쇄.
...
5. **안드로이드 모바일 통합 앱 (`android/voicecapSMS/`)**:
   ...
   - 실기기 설치 파일: `android/voicecapSMS/build/voicecap-sms-v1.3.1-install.apk` (917 KB).
   - 스토어 배포 번들: `app/build/outputs/bundle/release/app-release.aab` (520 KB).
